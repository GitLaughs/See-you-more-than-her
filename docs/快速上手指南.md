...
| 脚本 | 功能 | 使用频率 |
|------|------|---------|
| `build_src_all.sh` | **全量构建**：SDK + Demo + ROS2 | 首次构建 ⭐ |
| `build_incremental.sh` | **增量构建**：仅重编修改部分 | 日常开发 ⭐⭐⭐ |
| `build_ros2_ws.sh` | ROS2 工作区专项构建 | ROS 包开发 |
| `build_docker.sh` | Docker 容器内构建辅助脚本 | CI/CD |
| `install_ros2_jazzy.sh` | 容器内 ROS2 自动安装 | 环境配置 |
| `collect_evb_artifacts.sh` | 收集 EVB 固件产物 | 编译后收集 |

#### 📁 `src/` - 源代码 💻

```
src/
├── a1_ssne_ai_demo/          # 主应用（C++）
│   ├── demo_face.cpp         # 人脸检测 Demo (ssne_ai_demo)
│   ├── demo_vision.cpp       # 综合视觉 Demo (ssne_vision_demo) ⭐
│   ├── include/              # 头文件
│   │   ├── project_paths.hpp # 全局配置（路径、阈值、端口）
│   │   ├── yolov8_detector.hpp # YOLOv8 检测器
│   │   ├── osd_visualizer.hpp  # OSD 多层可视化
│   │   └── debug_data_interface.hpp # TCP 调试接口
│   ├── src/                  # 实现文件
│   ├── third_party/          # 第三方库（rplidar_sdk）
│   ├── app_assets/           # 资源文件（模型、LUT）
│   └── CMakeLists.txt
│
├── ros2_ws/                  # ROS2 工作区
│   ├── src/                  # ROS 包源码
│   │   ├── base_control_ros2/       # STM32 AKM UART 驱动 ⭐
│   │   ├── hardware_driver/         # RPLidar 驱动
│   │   ├── a1_robot_stack/          # 硬件 bringup
│   │   ├── wheeltec_robot_msg/      # 消息定义
│   │   ├── turn_on_wheeltec_robot/  # 启动配置
...
│   └── README.md             # 通信协议和硬件说明
│
└── buildroot_pkg/            # Buildroot 包定义（废弃）
```

**高频修改目录**：
- ⭐⭐⭐ `src/a1_ssne_ai_demo/src/` - 业务逻辑修改
- ⭐⭐⭐ `src/a1_ssne_ai_demo/include/project_paths.hpp` - 配置参数
- ⭐⭐ `src/ros2_ws/src/base_control_ros2/` - 底盘驱动修改

**禁止随意修改**：
- ❌ `src/a1_ssne_ai_demo/third_party/` - 第三方库源码
- ❌ `src/ros2_ws/src/serial/` - 外部依赖库

#### 📁 `tools/` - 开发工具

| 子目录 | 功能 | 使用场景 |
|--------|------|---------|
| `aurora/` | **Aurora 伴侣工具** | 点云/障碍/检测可视化 + CH347 固件烧录 ⭐⭐⭐ |
...
**脚本执行流程**：
1. ⏳ 构建 SDK 基础库（约 10 分钟）
2. ⏳ 构建 ssne_ai_demo（人脸检测 Demo）
3. ⏳ 构建 ssne_vision_demo（综合视觉 Demo）
4. ⏳ 构建 ROS2 工作区（约 5 分钟）
5. ⏳ 收集 EVB 产物到 `output/evb/`

**编译产物位置**：
```
output/evb/zImage.smartsens-m1-evb  ← A1 开发板固件
...
# 烧录指定固件
.\launch.ps1 -Flash -Firmware "D:\custom_fw.bin"
```
...
```bash
# 查看 SDK 编译日志
docker exec A1_Builder bash -lc "cat /app/smartsens_sdk/A1_SDK_SC132GS/smartsens_sdk/build.log"
...
- 调试数据：通过 TCP 9090 连接 Aurora 伴侣工具
...
2. 📖 阅读 `src/a1_ssne_ai_demo/README.md` - 理解主应用架构
3. 📖 阅读 `docs/编译手册.md` - 深入理解编译流程
4. 🔍 浏览核心代码：
   - `src/a1_ssne_ai_demo/demo_vision.cpp` - 主循环
   - `src/a1_ssne_ai_demo/include/project_paths.hpp` - 配置参数
...
| **编译流程** | `docs/编译手册.md` | ⭐⭐⭐ |
| **架构设计** | `docs/双板架构设计.md` | ⭐⭐ |
| **主应用说明** | `src/a1_ssne_ai_demo/README.md` | ⭐⭐ |
