...
```cpp
// 图像尺寸配置（根据镜头参数修改）
int img_width = 720;     // SC132GS 输入图像宽度
int img_height = 1280;   // SC132GS 输入图像高度
```
...
**计算 crop 参数，保证 resize 比例不变**

```cpp
// 图像处理器初始化
array<int, 2> img_shape = {720, 1280};     // 原始图像尺寸
array<int, 2> crop_shape = {720, 540};     // 裁剪尺寸
int crop_offset_y = 370;                    // Y方向裁剪偏移

// SC132GS: 720×1280 → 裁剪为 720×540 (上下各裁剪)
// pipe0设置：OnlineSetCrop(kPipeline0, 0, 720, 370, 910);
// 输出 720×540 灰度图
OnlineSetOutputImage(kPipeline0, format_online, 720, 540);
```
...
if (det_result->boxes.size() > 0) {
    ...
    // 坐标转换：crop图坐标 → 原图坐标
    std::vector<std::array<float, 4>> boxes_original;
    for (size_t i = 0; i < det_result->boxes.size(); i++) {
        float x1 = det_result->boxes[i][0] + crop_offset_x;
        float y1 = det_result->boxes[i][1];
        float x2 = det_result->boxes[i][2] + crop_offset_x;
        float y2 = det_result->boxes[i][3];
        boxes_original.push_back({x1, y1, x2, y2});
    }

    // OSD绘制检测框
    visualizer.Draw(boxes_original);
} else {
...
2. **裁剪参数**: 需要根据实际镜头分辨率计算 crop 参数，确保 resize 后图像比例不变
3. **640×480 训练数据**: 使用 `tools/aurora/aurora_capture.py` 拍摄 640×480 灰度图作为训练集
4. **量化推理**: 量化推理直接处理 0-255 范围图像数据，不需要额外配置 normalize 参数
