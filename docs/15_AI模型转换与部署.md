...
A1 主线部署输入始终是 `720×1280 -> 720×540 -> 640×480`；不要把 `640×640 letterbox` 当成最终效果基准。
...
array<int, 2> det_shape = {640, 480};  // 检测模型输入尺寸（中心裁剪 720x540 后再缩放）
string path_det = "/app_demo/app_assets/models/best_a1_640x480.m1model";
```
...
- 原始输入：SC132GS `720×1280` 灰度图
- 板端在线裁剪：`y=370..910`，得到 `720×540`
- NPU 输入：缩放到 `640×480`
- OSD 坐标回写：检测框 `y` 坐标需要加回裁剪偏移 `370`

```cpp
// 图像处理器初始化
array<int, 2> img_shape = {1280, 720};  // 原始图像尺寸（W×H）
// pipeline_image.cpp 内部使用 OnlineSetCrop:
// x=0..720, y=370..910，得到 720×540
// RunAiPreprocessPipe 再将裁剪结果缩放到 640×480
```

> **注意**：当前主线不是 `1280×720 -> 640×360` 直接缩放，
> 而是 `720×1280 -> 720×540 -> 640×480` 中心裁剪链路。
> 后处理坐标先还原到 `720×540` 裁剪空间，再在 OSD 阶段加回 `y` 偏移 `370`。
...
4. **训练/验证一致性**: 训练集、PC 预览验证、A1 板端前处理必须统一到 `720×1280 -> 720×540 -> 640×480`
