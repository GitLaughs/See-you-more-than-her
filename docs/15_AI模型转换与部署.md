...
Python 验证脚本里的 `640×640` 随机输入只用于验证 **裁剪前后后处理数值一致性**。若最终 A1 部署主线使用 `640×480` 裁剪链路，则训练、PC 预览验证、A1 前处理都要统一到 `720×1280 -> 720×540 -> 640×480`，不能再按 `640×360` 或 `640×640 letterbox` 当成最终效果基准。
...
2. **主线前处理**: `720×1280` 原图中心裁剪到 `720×540`，再缩放到 `640×480`
3. **训练/验证一致性**: 训练集、PC 预览验证、A1 板端前处理必须统一这条裁剪链路
4. **640×480 训练数据**: 使用 `tools/aurora/aurora_companion.py` 保存训练图时生成 `640×480` 中心裁剪灰度图
