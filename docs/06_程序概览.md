...
| 人脸推理 | `src/scrfd_gray.cpp` | SCRFD 灰度检测，RunAiPreprocessPipe 缩放 →640×360 |
...
| 类声明 | `include/common.hpp` | IMAGEPROCESSOR / SCRFDGRAY / FaceDetectionResult |
...
```
每帧处理 (main 循环):
  1. IMAGEPROCESSOR::GetImage()  → 采集 1280×720 Y8 全分辨率帧
  2. SCRFDGRAY::Predict()        → RunAiPreprocessPipe 缩放至 640×360 → SSNE NPU 推理
                                   Postprocess scale=2.0 → 坐标映射回 1280×720
  3. 决策与底盘控制：
     - 检测到人脸 → ChassisController::SendVelocity(100, 0, 0)  前进 100 mm/s
     - 未检测到人脸 → ChassisController::SendVelocity(0, 0, 0)  停车
