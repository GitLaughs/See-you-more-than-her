...
void IMAGEPROCESSOR::Initialize(std::array<int, 2>* in_img_shape) {
    ...
    if (OpenOnlinePipeline(kPipeline0)) {
        fprintf(stderr, "[IMAGEPROCESSOR] OpenOnlinePipeline failed");
        return;
    }
    if (OnlineSetOutputImage(kPipeline0, format_online, img_shape[0], img_shape[1])) {
        fprintf(stderr, "[IMAGEPROCESSOR] OnlineSetOutputImage failed");
    }
    if (UpdateOnlineParam()) {
        fprintf(stderr, "[IMAGEPROCESSOR] UpdateOnlineParam failed");
    }
}

void IMAGEPROCESSOR::GetImage(ssne_tensor_t* img_sensor) {
    int ret = GetImageData(img_sensor, kPipeline0, kSensor0, true);
    if (ret != SSNE_ERRCODE_NO_ERROR) {
        fprintf(stderr, "[IMAGEPROCESSOR] GetImageData failed: %d", ret);
    }
}
