...
本项目是基于 SmartSens SSNE (SmartSens Neural Engine) 的 AI 演示程序，当前主线使用 YOLOv8 灰度 4 类检测模型，覆盖人物、手势和障碍物检测。项目使用 C++ 开发，集成了图像处理、AI 模型推理和可视化显示等功能。

## 文件结构

```
ssne_ai_demo/
├── demo_face.cpp              # 主演示程序 - YOLOv8 检测演示
├── include/                   # 头文件目录
│   ├── common.hpp            # 公共数据结构定义
│   ├── utils.hpp             # 工具函数声明
...
│   ├── yolov8_gray.cpp      # YOLOv8 灰度检测模型实现
...
### 1. 主程序文件
- **demo_face.cpp**: 主演示程序，实现完整的 YOLOv8 检测流程
  - 初始化SSNE引擎
  - 配置图像处理器和检测模型
  - 主循环处理图像帧
  - 坐标转换和可视化显示
...
### 2. 核心类定义
- **common.hpp**: 定义核心数据结构
  - `FaceDetectionResult`: 检测结果结构体
  - `IMAGEPROCESSOR`: 图像处理器类
  - `YOLOV8`: YOLOv8 灰度检测模型类
...
- **src/yolov8_gray.cpp**: YOLOv8 灰度检测模型实现
  - 模型初始化和推理
  - DFL 解码、sigmoid、NMS 后处理
  - 640×480 输入对应 80×60 / 40×30 / 20×15 三个 FPN 尺度
...
- **模型推理** (`src/yolov8_gray.cpp`)
  ...
  - 通过 `ssne_getoutput()` 获取6个输出tensor（3个分类 head + 3个回归 head）

- **后处理** (`src/yolov8_gray.cpp`)
  - **DFL 解码**: 将 head 输出转换为 640×480 输入空间下的检测框
  - **置信度过滤**: 去除置信度低于阈值的检测框
  - **NMS**: 非极大值抑制，去除重叠的检测框
  - **尺度恢复**: 将坐标从640×480恢复到720×540
...
### 2. 离线处理（Offline Processing）- YOLOV8

离线处理在 `YOLOV8` 类中完成，主要负责AI模型的输入准备、NPU 推理和 CPU 后处理。整个离线处理流程包括预处理管道获取、720×540 到 640×480 的输入缩放、head6 输出解码以及资源释放。
