...
def parse_args():
    parser = argparse.ArgumentParser(description="Export MobileNet sigmoid classifier to ONNX")
    parser.add_argument("--checkpoint", required=True, help="Path to PyTorch checkpoint")
    parser.add_argument("--output_path", required=True, help="Path to exported ONNX model")
...
def main():
    ...
    export_model = SigmoidOnnxWrapper(model).eval()
    dummy_input = torch.randn(1, 3, MODEL_IMAGE_SIZE, MODEL_IMAGE_SIZE, dtype=torch.float32)
