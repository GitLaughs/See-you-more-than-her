...
def _suppress_opencv_logs():
    ...
    if original_level is not None:
        try:
            import logging
            logging.getLogger('cv2').setLevel(original_level)
        except Exception:
            pass
...
def probe_camera_device(device_id: int) -> dict:
    ...
    source = CAMERA_SOURCE_A1 if is_grayscale else CAMERA_SOURCE_WINDOWS
    ...
    return {
        ...
        "has_content": True,
        "supports_gray_fourcc": is_grayscale,
        "source": source,
...
def list_camera_devices(max_scan: int = MAX_DEVICE_SCAN) -> list:
    """简化版设备列表，只检查0和1号设备"""
    devices = []
    for i in range(min(max_scan, 2)):  # 只检查 0 和 1 号设备
        ...
        if requested_device < 0:
            if candidates:
                ...
                for c in candidates:
                    ...
                    print(f"  - device {c['id']}: {c['actual_width']}x{c['actual_height']} {kind} source={c.get('source', CAMERA_SOURCE_WINDOWS)}")
...
def camera_devices():
    ...
    bootstrapping = False
    if refresh or not camera_devices_snapshot:
        if camera_bootstrap_active:
            devices = []
            bootstrapping = True
        else:
            print("[INFO] 重新扫描摄像头设备...")
            devices = list_camera_devices()
            camera_devices_snapshot = list(devices)
            print(f"[INFO] 扫描完成，找到 {len(devices)} 个设备")
    else:
    ...
    for d in devices:
        ...
        label = f"device {d['id']} - {d['actual_width']}x{d['actual_height']} {kind}"
        items.append({
            "id": d["id"],
            "label": label,
